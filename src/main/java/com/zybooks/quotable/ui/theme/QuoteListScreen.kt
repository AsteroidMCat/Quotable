...
//    LazyColumn(modifier= Modifier.padding(start=0.dp).fillMaxHeight()) {
//        items(
//            items = getQuotesList(),
        ...
        confirmValueChange = {
            when(it){
                SwipeToDismissBoxValue.EndToStart ->{
                    ...
                    Toast.makeText(context, "${quote.author}'s quote deleted",
                        Toast.LENGTH_SHORT).show()
                    true
                }
                SwipeToDismissBoxValue.StartToEnd->{
                    toggleFavoriteQuote(currentItem)
                    Toast.makeText(context, "Quote added to favorites",
                        Toast.LENGTH_SHORT).show()
                    false
                }
                else -> false
